---
title: "In Detail: Engines - Reportelement"
subtitle: "Creating atomic report elements for summaries and visuals"
author: "mwiller"
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{In Detail: Engines - Reportelement}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

> **Summary**  
> This vignette covers the **reportelement engines**.  
> It describes how atomic report components—such as tables, text snippets, and plots—are generated, which can then be composed into larger reports and published via other engines.
...
|---------------------|--------------|---------------------------------------------------------------|
| `workflow_results`  | list         | Output from completed workflow (⚠️ provided by the framework) |
| `split_output`      | list         | Splitting information from workflow (⚠️ provided by the framework) |
...
## 🔗 Relationship to Report and Publishing Engines
...
You can use reportelements directly—for instance, to generate and compare plots for internal review—without having to define a full report. However, Reports always build on one or more Reportelements, and Publishing always requires at least a Reportelement or Report.

---

## ⚙️ Available Reportelement Engines

| Engine Name                           | Function Name                                      | Description                              | Template Path                                                                             | Detail Page |
|----------------------------------------|----------------------------------------------------|------------------------------------------|--------------------------------------------------------------------------------------------|-------------|
| `reportelement_text_msesummary`       | `engine_reportelement_text_msesummary()`          | Textual summary of MSE values           | `inst/templates_control/8_a_engine_reportelement_text_msesummary.R`                     | [📄](detail_engine_reportelement_text_msesummary.html) |
| `reportelement_table_splitmetrics`    | `engine_reportelement_table_splitmetrics()`       | Table summarizing metrics per split     | `inst/templates_control/8_b_engine_reportelement_table_splitmetrics.R`                  | [📄](detail_engine_reportelement_table_splitmetrics.html) |
| `reportelement_boxplot_predictions`   | `engine_reportelement_boxplot_predictions()`      | Boxplot of predicted values by group    | `inst/templates_control/8_c_engine_reportelement_boxplot_predictions.R`                 | [📄](detail_engine_reportelement_boxplot_predictions.html) |
...
## 🛠️ Writing Your Own Reportelement Engine
...
## 🎯 Possible Use-Case: Modular Report Assembly

Each reportelement serves a specific communication purpose. For example, the engine reportelement_text_msesummary generates a **compact text-based summary of the mean squared error (MSE)** across all splits. This is particularly helpful when the focus is on presenting a concise numerical evaluation—e.g., in executive summaries, automated model selection reports, or dashboards where interpretability and brevity are prioritized.
