...
wrapper_eval_mae <- function(control) {
  ...
  params <- merge_with_defaults(control$params$evaluation$params,
                                default_params_eval_mae())
  eval_data <- control$params$evaluation$eval_data
  ...
  initialize_output_eval(
    ...
    protected_attributes = control$params$evaluation$protected_attributes,
    params               = params,
...
For transparency and reproducibility, a complete worked example of 
LLM-assisted engine development is included in this package.  
The demonstration archives the full process — prompt, conversation transcript, 
generated engine, provenance logs, and a workflow run — for the evaluation 
engine **eval_median**.

You can explore it via the dedicated vignette:

- **Vignette:** *LLM Demonstration: Eval Median*(llm_demonstration_eval_median.html)
- **Files:** accessible under  
  ```r
  system.file("llm_demonstrations/2025-08-29_eval_median", package = "flowengineR")
  ```