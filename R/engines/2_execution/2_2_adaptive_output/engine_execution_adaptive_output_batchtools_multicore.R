...
#'
#' Executes batches of 1-split workflows in parallel using `batchtools` (multicore backend)
  ...
  repeat {
    ...
    if (length(metric_values) >= params$min_splits) {
      ...
      if (isTRUE(stability_result$is_stable)) {
        message(sprintf(
          "[ADAPTIVE-BT] Stability reached (%s): %.4f < %.4f after %d splits. Stopping.",
      ...
      if (length(metric_values) >= params$max_splits) {
        message(sprintf("[ADAPTIVE-BT] Maximum number of splits (%d) reached. Stopping.", params$max_splits))
        break
      }
    }
