---
title: "Metalevel Flowchart"
subtitle: "Visualizing the orchestration of engines in workflows"
author: "mwiller"
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{Metalevel Flowchart}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

> **Summary**  
> This vignette presents the **metalevel flowchart** of `flowengineR`.  
> It visualizes how engines are orchestrated across different stages (splitting, training, fairness, evaluation, reporting, publishing) and clarifies the standardized data flow between them.

---

This vignette visualizes the internal structure of the `flowengineR` framework.  
The flowchart highlights how user input is routed through controllers and engines, how outputs are standardized, and how different parts of the modular workflow interact.
...
knitr::include_graphics(system.file("extdata", "metalevel_flowchart.png", package = "flowengineR"))
```

*Figure: Full modular workflow as a flowchart exported from Mermaid.*

---

## 🧾 Mermaid Source Code

> This code can be copied into [https://mermaid.live](https://mermaid.live) to edit or regenerate the diagram.
...
graph TD
    ...
    Input(Control Object):::object ==> B[run_workflow-function]
    ...
    %% run_workflow internal structure
    ...
    Dec9{Manuell restart after external calculation like with SLURM needed?}:::decider
                        ...
                        E[function: resume_workflow]:::controller_style --> B2
                        Ans18 -->Hlp2

    Hlp2[Helper: Prepare Resume Object, Engine specific needed]:::helper_style --> C11

    %% continue_workflow internal structure
    subgraph continue_workflow
        C11 ==> B2[continue_workflow-function]
    ...
    %% Workflow inside run_workflow_singlesplitloop
    subgraph Workflow in run_workflow_singlesplitloop
        direction TB
        E10 -->|splited dataset executed as the engine specifies, can be outside of R like with SLURM| D[run_workflow_singlesplitloop]
        D[run_workflow_singlesplitloop] -->|raw data| Dec2{Pre-Precessing Active?}:::decider
            Dec2 -->Ans3((Yes)):::yes
            Dec2 -->Ans4((No)):::no

        %% Embedded Pre-Processing Engine
        subgraph Pre-Processing Engines
        direction TB
            Ans3 -->|data| FPr1[Standardized Inputs: data, protected_attributes, target_var, params]:::input_style
            FPr1 --> E2[Pre-Precessing Engine]:::engine
            C2[function: controller_preprocessing]:::controller_style -->|rest| FPr1
            E2 --> OF2[function: initialize_output_preprocessing]:::output_style
        ...
        subgraph Training Process
            ...
            T2 -->|model| Dec3{In-Precessing Active?}:::decider
            Dec3 -->Ans5((Yes)):::yes
            ...
            direction TB
                ...
                FIn1 --> E4[In-Precessing Engine]:::engine
                C4[function: controller_inprocessing]:::controller_style -->|rest| FIn1
                E4 --> OF4[function: initialize_output_inprocessing]:::output_style
        ...
        %% Integrated Post-Processing Subgraph
        subgraph Post-Processing Engines
            direction TB
            Ans7 -->|postprocessing_data including Raw Predictions| FP1[Standardized Inputs: postprocessing_data, params, protected_name]:::input_style
            FP1 --> E5[Post-Precessing Engine]:::engine
            C5[function: controller_postprocessing]:::controller_style -->|rest| FP1
            E5 --> OF5[function: initialize_output_postprocessing]:::output_style
        ...
        RP --> Dec4{Post-Precessing Active?}:::decider
            Dec4 -->Ans7((Yes)):::yes
            Dec4 -->Ans8((No)):::no
...
- To edit the diagram, paste the Mermaid code into [https://mermaid.live](https://mermaid.live)
- To update the image: export a new PNG and overwrite `inst/extdata/metalevel_flowchart.png`
- This PNG is included with the installed package and shown above
