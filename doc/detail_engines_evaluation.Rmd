---
title: "In Detail: Engines - Evaluation"
subtitle: "Overview and structure of evaluation engines"
author: "mwiller"
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{In Detail: Engines - Evaluation}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

> **Summary**  
> This vignette provides an overview of **evaluation engines**.  
> It introduces their standardized input/output design, explains how different evaluation methods can be plugged into workflows, and illustrates how metrics are returned and documented.
...
|--------------------|------------------|-------------------------------------------------------------------|
| `eval_data`        | data.frame       | Data containing predictions and reference values (⚠️ provided by the framework) |
| `params`           | list             | Additional parameters for the evaluation                          |
| `protected_name`   | character        | Name(s) of protected attributes (⚠️ provided by the framework)  |
...
## ⚙️ Available Evaluation Engines

| Engine Name              | Function Name                           | Description                                    | Template Path                                                                  | Detail Page |
|--------------------------|------------------------------------------|------------------------------------------------|----------------------------------------------------------------------------------|--------------|
| `evaluate_mse`           | `engine_evaluation_mse()`               | Mean squared error evaluation                  | `inst/templates_control/7_2_a_template_control_evaluation_mse.R`                 | [📄](detail_engine_evaluation_mse.html) |
| `evaluation_statisticalparity`       | `engine_evaluation_statisticalparity()` | Statistical parity difference (SPD)            | `inst/templates_control/7_3_a_template_control_evaluation_statisticalparity.R`   | [📄](detail_engine_evaluation_statisticalparity.html) |
| `evaluate_summarystats`  | `engine_evaluation_summarystats()`      | Summary statistics (mean, sd, etc.)            | `inst/templates_control/7_1_a_template_control_evaluation_summarystats.R`        | [📄](detail_engine_evaluation_summarystats.html) |
...
## 🛠️ Writing Your Own Evaluation Engine
...
## 🎯 Possible Use-Case: Combined Metric Tracking
